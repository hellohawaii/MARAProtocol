...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    prev_sand = copy.deepcopy(state.get("sand", []))
    prev_water = copy.deepcopy(state.get("water", []))
    sand_button = copy.deepcopy(state.get("sandButton", []))
    water_button = copy.deepcopy(state.get("waterButton", []))
    click_type = str(hidden_state.get("clickType", "sand"))
