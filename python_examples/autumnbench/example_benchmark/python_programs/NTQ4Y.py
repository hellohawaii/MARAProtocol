...
def init_state():
    ...
    state = {
        "GRID_SIZE": grid_size,
        "vesselButton": _button_cells(2, 0, "purple"),
        "plugButton": _button_cells(5, 0, "orange"),
        "waterButton": _button_cells(8, 0, "blue"),
        "removeButton": _button_cells(11, 0, "gray"),
        "clearButton": _button_cells(14, 0, "red"),
        "vessels": [{"position": {"x": x, "y": y}, "color": "purple"} for x, y in vessels_positions],
        "plugs": [{"position": {"x": x, "y": y}, "color": "orange"} for x, y in plugs_positions],
        "waterList": [],
    }
...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    prev_vessels = copy.deepcopy(state.get("vessels", []))
    prev_plugs = copy.deepcopy(state.get("plugs", []))
    prev_water = copy.deepcopy(state.get("waterList", []))

    vessel_button = copy.deepcopy(state.get("vesselButton", _button_cells(2, 0, "purple")))
    plug_button = copy.deepcopy(state.get("plugButton", _button_cells(5, 0, "orange")))
    water_button = copy.deepcopy(state.get("waterButton", _button_cells(8, 0, "blue")))
    remove_button = copy.deepcopy(state.get("removeButton", _button_cells(11, 0, "gray")))
    clear_button = copy.deepcopy(state.get("clearButton", _button_cells(14, 0, "red")))
    ...
    if _action_type(action) == "click" and isinstance(action, dict):
        ...
        if _is_free_pos(click_pos, current_occupied, grid_size) and current_particle == "plug":
            ...
            updated_vars.add("plugs")

        if _is_free_pos(click_pos, current_occupied, grid_size) and current_particle == "water":
    ...
    if _clicked_obj(action, clear_button):
        ...
        current_plugs = []
        current_water = []
        updated_vars.update({"vessels", "plugs", "waterList"})

    next_state = {
        "GRID_SIZE": grid_size,
        "vesselButton": vessel_button,
        "plugButton": plug_button,
        "waterButton": water_button,
        "removeButton": remove_button,
        "clearButton": clear_button,
        "vessels": current_vessels,
        "plugs": current_plugs,
        "waterList": current_water,
    }
