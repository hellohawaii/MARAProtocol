...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    prev_mario_cells = copy.deepcopy(state.get("mario", []))
    prev_steps_cells = copy.deepcopy(state.get("steps", []))
    prev_coins_cells = copy.deepcopy(state.get("coins", []))
    prev_enemy_cells = copy.deepcopy(state.get("enemy", []))
    prev_bullets_cells = copy.deepcopy(state.get("bullets", []))
    ...
    hidden_steps = hidden_state.get("steps")
    ...
    if _intersects(prev_mario_cells, prev_coins_cells):
        ...
        updated_vars.update({"coins", "mario"})
    ...
    if action_type == "click" and prev_mario_bullets > 0 and prev_mario_origin is not None:
        ...
        current_mario_bullets = prev_mario_bullets - 1
        updated_vars.update({"bullets", "mario"})
    ...
    if prev_enemy_defined and _intersects(prev_enemy_cells, prev_bullets_cells):
        ...
        updated_vars.update({"bullets", "enemy", "enemyLives"})

    # --- next updates (initnext) ---
    if "mario" not in updated_vars:
    ...
    if "coins" not in updated_vars:
        current_coins_cells = copy.deepcopy(prev_coins_cells)

    if "enemy" not in updated_vars:
    ...
    if "bullets" not in updated_vars:
        current_bullets_cells = []
    ...
    next_state = {
        "GRID_SIZE": grid_size,
        "mario": current_mario_cells,
        "steps": _render_steps(current_steps_hidden),
        "coins": current_coins_cells,
        "enemy": current_enemy_cells,
        "bullets": current_bullets_cells,
    }

    next_hidden = {
        "mario_bullets": int(current_mario_bullets),
        "steps": current_steps_hidden,
