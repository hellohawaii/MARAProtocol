...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    prev_leaves = copy.deepcopy(state.get("leaves", []))
    prev_sun_cells = copy.deepcopy(state.get("sun", []))
    prev_cloud_cells = copy.deepcopy(state.get("cloud", []))
    ...
    new_state = {
        "GRID_SIZE": grid_size,
        "sun": current_sun_cells,
        "cloud": current_cloud_cells,
        "water": current_water,
