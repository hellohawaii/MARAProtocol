...
def init_state():
    ...
    state = {
        ...
        "water": water_cells,
        "rocks": [],
        "crate": _render_crate(crate_origin),
    }
...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    prev_water_cells = copy.deepcopy(state.get("water", []))
    prev_rocks_cells = copy.deepcopy(state.get("rocks", []))
    prev_crate_cells = copy.deepcopy(state.get("crate", []))
    ...
    if action_type == "click" and isinstance(action, dict):
        ...
        if _is_free_pos(click_pos, prev_occupied, grid_size):
            ...
            updated_vars.add("rocks")

    # --- next updates ---
    if "crate" not in updated_vars:
    ...
    next_state = {
        "GRID_SIZE": grid_size,
        "water": current_water_cells,
        "rocks": current_rocks_cells,
        "crate": _render_crate(current_crate_origin),
    }
