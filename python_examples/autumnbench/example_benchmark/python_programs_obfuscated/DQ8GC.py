...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    prev_inactive = copy.deepcopy(state.get("m6K", []))
    prev_active_list = copy.deepcopy(state.get("2yW", []))
    ...
    if prev_active is not None:
        ...
        if active_pos is not None:
            ...
            if any_unhealthy:
                ...
                new_state["2yW"] = [current_active]
                updated_vars.add("2yW")
    ...
    if click_pos is not None:
        ...
        if clicked_particle is not None:
            ...
            new_state["m6K"] = remaining
            new_state["2yW"] = [clicked_particle]
            current_active = clicked_particle
            updated_vars.update({"m6K", "2yW"})
    ...
    if "m6K" not in updated_vars:
        ...
        new_state["m6K"] = next_inactive

    # Normalize colors for visibility.
    if "m6K" in new_state and isinstance(new_state["m6K"], list):
        for obj in new_state["m6K"]:
            _set_particle_health(obj, _particle_health(obj))
    if "2yW" in new_state and isinstance(new_state["2yW"], list):
        for obj in new_state["2yW"]:
            _set_particle_health(obj, _particle_health(obj))
