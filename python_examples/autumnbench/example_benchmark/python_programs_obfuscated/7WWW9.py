...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    prev_mobile = copy.deepcopy(state.get("9ZM", []))
    fixed_magnet = copy.deepcopy(state.get("aNF", []))

    action_type = _action_type(action)

    # On-clauses: action-controlled movement (uses prev mobile magnet).
    if action_type == "left":
        new_state["9ZM"] = _move_no_collision(prev_mobile, -1, 0, fixed_magnet)
    elif action_type == "right":
        new_state["9ZM"] = _move_no_collision(prev_mobile, 1, 0, fixed_magnet)
    elif action_type == "up":
        new_state["9ZM"] = _move_no_collision(prev_mobile, 0, -1, fixed_magnet)
    elif action_type == "down":
        new_state["9ZM"] = _move_no_collision(prev_mobile, 0, 1, fixed_magnet)

    current_mobile = new_state.get("9ZM", [])

    # Repulsion: like poles adjacent reset to previous step.
    if _adjacent_elem(_pos_pole(current_mobile), _pos_pole(fixed_magnet)):
        new_state["9ZM"] = copy.deepcopy(prev_mobile)
    ...
    if delta is not None and delta in attract_vectors:
        ...
        new_state["9ZM"] = _shift_cells(current_mobile, dx, dy)
