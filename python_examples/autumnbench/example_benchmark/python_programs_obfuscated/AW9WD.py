...
def _hidden_from_state(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    eggshells = []
    for cell in state.get("6n9", []) if isinstance(state, dict) else []:
        pos = _cell_pos(cell)
        if pos is None:
            continue
...
def init_state():
    ...
    state = {
        "GRID_SIZE": grid_size,
        "zT0": feathers,
        "6n9": _render_eggshells(eggshells),
    }
    hidden_state = {
        "6n9": eggshells,
        "zT0": feathers,
...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    new_state = {
        "GRID_SIZE": grid_size,
        "zT0": copy.deepcopy(prev_feathers),
        "6n9": _render_eggshells(updated_eggshells),
    }

    next_hidden = {
        "6n9": updated_eggshells,
        "zT0": prev_feathers,
    }
