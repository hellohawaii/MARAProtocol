...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    left_button = copy.deepcopy(state.get("fu4", []))
    right_button = copy.deepcopy(state.get("efR", []))
    up_button = copy.deepcopy(state.get("p2n", []))
    down_button = copy.deepcopy(state.get("gwB", []))

    prev_blob_cells = copy.deepcopy(state.get("9Ew", []))
