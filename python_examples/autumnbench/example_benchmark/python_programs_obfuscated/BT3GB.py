...
def init_state():
    ...
    state = {
        "GRID_SIZE": grid_size,
        "TZM": _celestial_cells(celestial_origin, day),
...
def predict_dynamics(state: Dict[str, Any], hidden_state: Dict[str, Any] | None, action: Any):
    ...
    prev_water = hidden_state.get("LrC")
    ...
    if action_type == "click":
        ...
        updated_vars.add("TZM")
        updated_vars.add("LrC")

    # --- next updates ---
    if "TZM" not in updated_vars:
        current_day = prev_day
    ...
    new_state = {
        "GRID_SIZE": grid_size,
        "TZM": _celestial_cells(prev_celestial_origin, current_day),
        "gUe": _cloud_cells(current_cloud_origin),
        "LrC": [_water_cell(drop["x"], drop["y"], drop.get("liquid", False)) for drop in current_water],
    }
